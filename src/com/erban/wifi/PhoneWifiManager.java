...
public class PhoneWifiManager {
	...
	public List<PhoneWifiInfo> getConfiguredWifis() {
		WifiInfo wifiInfo = wifiManager.getConnectionInfo();
		List<WifiConfiguration> configurations = wifiManager
				.getConfiguredNetworks();
		...
		List<PhoneWifiInfo> wifiInfos = new ArrayList<PhoneWifiInfo>();
		for (WifiConfiguration item : configurations) {
			wifiInfos.add(new WifiInfoWrapper(wifiInfo, item));
		}
	...
	public int getMatchedNetworkId(PhoneWifiInfo wifiInfo) {
		List<WifiConfiguration> configurations = wifiManager
				.getConfiguredNetworks();
		if (configurations == null) {
			return -1;
		...
		for (WifiConfiguration item : configurations) {
			if (wifiInfo.getWifiName().equals(WifiUtil.removeDoubleQuotes(item.SSID))) {
